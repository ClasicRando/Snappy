...
class BooleanListDecoder: Decoder<List<Boolean>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Boolean>? {
        return row.getArray(fieldName).toList()
    }
}

class ByteListDecoder: Decoder<List<Byte>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Byte>? {
        return row.getArray(fieldName).toList()
    }
}

class ShortListDecoder: Decoder<List<Short>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Short>? {
        return row.getArray(fieldName).toList()
    }
}

class IntListDecoder: Decoder<List<Int>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Int>? {
        return row.getArray(fieldName).toList()
    }
}

class LongListDecoder: Decoder<List<Long>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Long>? {
        return row.getArray(fieldName).toList()
    }
}

class FloatListDecoder: Decoder<List<Float>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Float>? {
        return row.getArray(fieldName).toList()
    }
}

class DoubleListDecoder: Decoder<List<Double>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Double>? {
        return row.getArray(fieldName).toList()
    }
}

class InstantListDecoder: Decoder<List<Instant>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Instant>? {
        return row.getArray(fieldName).toList()
    }
}

class StringListDecoder: Decoder<List<String>> {
    override fun decode(row: SnappyRow, fieldName: String): List<String>? {
        return row.getArray(fieldName).toList()
    }
}

class BigDecimalListDecoder: Decoder<List<BigDecimal>> {
    override fun decode(row: SnappyRow, fieldName: String): List<BigDecimal>? {
        return row.getArray(fieldName).toList()
    }
}

class DateListDecoder: Decoder<List<Date>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Date>? {
        return row.getArray(fieldName).toList()
    }
}

class TimestampListDecoder: Decoder<List<Timestamp>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Timestamp>? {
        return row.getArray(fieldName).toList()
    }
}

class TimeListDecoder: Decoder<List<Time>> {
    override fun decode(row: SnappyRow, fieldName: String): List<Time>? {
        return row.getArray(fieldName).toList()
    }
}

class PgObjectListDecoder: Decoder<List<PGobject>> {
    override fun decode(row: SnappyRow, fieldName: String): List<PGobject>? {
        return row.getArray(fieldName).toList()
    }
}

class BooleanArrayDecoder: Decoder<BooleanArray> {
    override fun decode(row: SnappyRow, fieldName: String): BooleanArray? {
...
class ByteArrayDecoder: Decoder<ByteArray> {
    override fun decode(row: SnappyRow, fieldName: String): ByteArray? {
        val value = row.getArray(fieldName)
        if (value.array == null) {
            return null
        }
...
class ShortArrayDecoder: Decoder<ShortArray> {
    override fun decode(row: SnappyRow, fieldName: String): ShortArray? {
        val value = row.getArray(fieldName)
        if (value.array == null) {
            return null
        }
...
class IntArrayDecoder: Decoder<IntArray> {
    override fun decode(row: SnappyRow, fieldName: String): IntArray? {
        val value = row.getArray(fieldName)
        if (value.array == null) {
            return null
        }
...
class LongArrayDecoder: Decoder<LongArray> {
    override fun decode(row: SnappyRow, fieldName: String): LongArray? {
        val value = row.getArray(fieldName)
        if (value.array == null) {
            return null
        }
...
class FloatArrayDecoder: Decoder<FloatArray> {
    override fun decode(row: SnappyRow, fieldName: String): FloatArray? {
        val value = row.getArray(fieldName)
        if (value.array == null) {
            return null
        }
...
class DoubleArrayDecoder: Decoder<DoubleArray> {
    override fun decode(row: SnappyRow, fieldName: String): DoubleArray? {
        val value = row.getArray(fieldName)
        if (value.array == null) {
            return null
        }
...
class InstantArrayDecoder: Decoder<Array<Instant>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Instant>? {
        return row.getArray(fieldName).toArray()
    }
}

class StringArrayDecoder: Decoder<Array<String>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<String>? {
        return row.getArray(fieldName).toArray()
    }
}

class BigDecimalArrayDecoder: Decoder<Array<BigDecimal>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<BigDecimal>? {
        return row.getArray(fieldName).toArray()
    }
}

class DateArrayDecoder: Decoder<Array<Date>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Date>? {
        return row.getArray(fieldName).toArray()
    }
}

class TimestampArrayDecoder: Decoder<Array<Timestamp>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Timestamp>? {
        return row.getArray(fieldName).toArray()
    }
}

class TimeArrayDecoder: Decoder<Array<Time>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Time>? {
        return row.getArray(fieldName).toArray()
    }
}

class NullableBooleanArrayDecoder: Decoder<Array<Boolean?>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Boolean?>? {
        return row.getArray(fieldName).toArrayWithNulls()
    }
}

class NullableByteArrayDecoder: Decoder<Array<Byte?>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Byte?>? {
        return row.getArray(fieldName).toArrayWithNulls()
    }
}

class NullableShortArrayDecoder: Decoder<Array<Short?>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Short?>? {
        return row.getArray(fieldName).toArrayWithNulls()
    }
}

class NullableIntArrayDecoder: Decoder<Array<Int?>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Int?>? {
        return row.getArray(fieldName).toArrayWithNulls()
    }
}

class NullableLongArrayDecoder: Decoder<Array<Long?>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Long?>? {
        return row.getArray(fieldName).toArrayWithNulls()
    }
}

class NullableFloatArrayDecoder: Decoder<Array<Float?>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Float?>? {
        return row.getArray(fieldName).toArrayWithNulls()
    }
}

class NullableDoubleArrayDecoder: Decoder<Array<Double?>> {
    override fun decode(row: SnappyRow, fieldName: String): Array<Double?>? {
        return row.getArray(fieldName).toArrayWithNulls()
    }
}
